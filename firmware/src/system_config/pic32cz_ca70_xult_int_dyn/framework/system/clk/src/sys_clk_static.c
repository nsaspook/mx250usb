...
void SYS_CLK_Initialize( const SYS_CLK_INIT const* clkInit )
{
    ...
    mainSetup.enableXtal = true;
    mainSetup.bypassXtal = false;
    mainSetup.enableRcOsc = false;
    mainSetup.rcOscFrequency = SYS_CLK_RC_FREQUENCY_4_MHZ;
    mainSetup.source = SYS_CLK_MAIN_SOURCE_XTAL;
    SYS_CLK_MainClockSetup ( &mainSetup );

    /* Setup PLLA */
    pllaSetup.enable = true;
    pllaSetup.multiplier = 25;
    pllaSetup.divider = 1;
    SYS_CLK_PLLASetup ( &pllaSetup );

    /* Setup USB PLL */
    upllSetup.enable = true;
    upllSetup.multiplier = 40;
    upllSetup.divider = 1;
    SYS_CLK_UPLLSetup ( &upllSetup );

    /* Setup Full-Speed USB clock output */
    usbClkSetup.enable = true;
    usbClkSetup.source = SYS_CLK_SOURCE_USB_PLL;
    usbClkSetup.divider = 10;
    SYS_CLK_USBClockSetup ( &usbClkSetup );

    /* Configure master clock controller */
    masterSetup.source = SYS_CLK_SOURCE_PLLA;
    masterSetup.prescaler = SYS_CLK_MASTER_PRESCALER_2;
    masterSetup.divider = SYS_CLK_MASTER_DIVIDER_1;
    SYS_CLK_MasterClockSetup ( &masterSetup );


    /* - Enable the peripheral clock selected by MHC/peripheral driver */
   	_PMC_REGS->PMC_PCER0.w=0x00031C00;
    _PMC_REGS->PMC_PCER1.w=0x00000004;
...
void SYS_CLK_MainClockSetup ( const SYS_CLK_MAIN_SETUP* setup )
{
    ...
    else if ( setup->enableXtal )
    {
        ...
        _PMC_REGS->CKGR_MOR.w = CKGR_MOR_KEY_PASSWD | CKGR_MOR_MOSCXTST(255) |
